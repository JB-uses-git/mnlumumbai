    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans">
      ...
      <nav className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <h1 className="text-xl font-serif font-bold text-primary">Journal Guidelines</h1>
        </div>
      </nav>
      ...
      <header className="relative w-full overflow-hidden bg-primary py-24 sm:py-32 flex items-center justify-center">
        ...
        <div className="relative z-10 container mx-auto px-6 text-center max-w-4xl flex flex-col items-center">
          ...
          <h1 className="text-4xl md:text-6xl font-serif font-extrabold text-white mb-6 tracking-tight">Journal Submission Guidelines</h1>
        ...
        >
          ...
          <motion.section variants={fadeInUp} className="bg-card rounded-3xl p-6 sm:p-8 md:p-12 shadow-sm border border-border/50 relative overflow-hidden group hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center gap-4 mb-8">
              <div className="w-16 h-16 rounded-2xl bg-secondary/20 text-secondary-foreground flex items-center justify-center flex-shrink-0">
                <FaBook size={28} />
              </div>
              <h2 className="text-2xl md:text-3xl font-serif font-bold text-primary">Categories</h2>
            </div>
            
            <div className="grid md:grid-cols-2 gap-6 mb-8">
              <div className="bg-muted/30 p-6 rounded-2xl border border-border/50">
                <h3 className="font-bold text-foreground text-lg mb-2">Long Form Articles</h3>
                <div className="text-sm text-primary font-bold mb-3">6,000 – 10,000 words</div>
                <p className="text-muted-foreground leading-relaxed">Submissions in this category are expected to engage with the topic, its theme, and available literature comprehensively so as to offer a novel and original reassessment.</p>
              </div>
              
              <div className="bg-muted/30 p-6 rounded-2xl border border-border/50">
                <h3 className="font-bold text-foreground text-lg mb-2">Essays</h3>
                <div className="text-sm text-primary font-bold mb-3">4,000 – 6,000 words</div>
                <p className="text-muted-foreground leading-relaxed">Comparatively more concise and limited in scope. Essays usually deal with a very specific topic as compared to Long Form Articles.</p>
              </div>

              <div className="bg-muted/30 p-6 rounded-2xl border border-border/50">
                <h3 className="font-bold text-foreground text-lg mb-2">Case/Legislative Comments</h3>
                <div className="text-sm text-primary font-bold mb-3">1,500 – 3,000 words</div>
                <p className="text-muted-foreground leading-relaxed">Expected to critically analyse a recent judicial/quasi-judicial decision or legislative/policy proposal and comment on its implications.</p>
              </div>

              <div className="bg-muted/30 p-6 rounded-2xl border border-border/50">
                <h3 className="font-bold text-foreground text-lg mb-2">Book Reviews</h3>
                <div className="text-sm text-primary font-bold mb-3">2,000 – 3,000 words</div>
                <p className="text-muted-foreground leading-relaxed">Expected to critically analyse and engage with a recent academic book that falls under our mandate.</p>
              </div>
            </div>
            <div className="p-4 bg-secondary/10 border-l-4 border-secondary rounded-r-xl">
              <p className="text-sm text-secondary-foreground/80">
                <strong className="font-bold">Note:</strong> These word limits are merely indicative and we retain a reasonable level of flexibility. Word limits are exclusive of footnotes.
              </p>
            </div>
          </motion.section>
          ...
          <motion.section variants={fadeInUp} className="bg-card rounded-3xl p-6 sm:p-8 md:p-12 shadow-sm border border-border/50 relative hover:shadow-md transition-shadow">
            ...
            <div className="grid gap-6">
              {[
                { title: 'Abstract', desc: 'All submissions must include an abstract of not more than 300 words, explaining the main idea, the objective of the article and the conclusions drawn from it.' },
                { title: 'Formatting', desc: 'The manuscript should be on A4-sized paper, in Times New Roman, font size 12, 1.5 line spacing, justified and 1-inch margins on each side. Footnotes should be in Times New Roman, font size 10 with single line spacing.' },
                { title: 'Citations', desc: 'References must be in the form of hyperlinks. Use of footnotes/endnotes should be limited. For footnotes/endnotes, the OSCOLA (4th edition) must be adhered to.' },
                { title: 'Anonymity', desc: 'The manuscript should NOT contain any identification of the author/s, which shall be a ground for rejection. Authors should provide their details in the covering letter / Google Form.' },
                { title: 'Approach', desc: 'Submissions should be analytical and solution oriented rather than descriptive. Co-authorship of up to 2 authors is permitted.' }
              ].map((item, i) => (
          ...
          <motion.section variants={fadeInUp} className="bg-gradient-to-br from-primary/5 to-transparent rounded-3xl p-6 sm:p-8 md:p-12 shadow-sm border border-primary/20 relative hover:shadow-md transition-shadow">
            ...
            <div className="grid sm:grid-cols-2 gap-6 mb-12">
              ...
              <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex flex-col gap-3">
                <h3 className="font-bold text-foreground text-lg flex items-center gap-2">
                  <FaExclamationCircle className="text-red-500" /> Plagiarism
                </h3>
                <p className="text-muted-foreground text-sm leading-relaxed">Any form of <strong className="text-red-600">plagiarism will lead to direct rejection</strong>. The relevant sources should be duly acknowledged.</p>
              </div>

              <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex flex-col gap-3">
                <h3 className="font-bold text-foreground text-lg flex items-center gap-2">
                  <FaCheckCircle className="text-primary" /> Confirmation
                </h3>
                <p className="text-muted-foreground text-sm leading-relaxed">Authors will receive a confirmation mail within 24 hours. If not received, please contact <a href="mailto:calstar@mnlumumbai.edu.in" className="text-primary hover:underline">calstar@mnlumumbai.edu.in</a></p>
              </div>

              <div className="bg-card p-6 rounded-2xl border border-border shadow-sm flex flex-col gap-3">
                <h3 className="font-bold text-foreground text-lg flex items-center gap-2">
                  <FaCheckCircle className="text-primary" /> Exclusivity
                </h3>
                <p className="text-muted-foreground text-sm leading-relaxed">Please do not send pieces that have already been submitted elsewhere for review.</p>
              </div>
            </div>
          ...
          <motion.section variants={fadeInUp} className="bg-card rounded-3xl p-6 sm:p-8 md:p-12 shadow-sm border border-border/50 relative hover:shadow-md transition-shadow mb-12">
            ...
            <div className="relative border-l-4 border-primary/20 ml-4 sm:ml-6 md:ml-8 space-y-12 pb-8 mt-12">
              {[
                ...
                { stage: 'Stage II', title: 'Substantive Review', desc: 'Evaluated for depth of analysis, originality of argumentation, and novelty of the topic. Reviewers assess whether the piece contributes meaningfully to existing discourse and aligns with the thematic focus of the Journal.' },
            ...
            <div className="mt-12 bg-muted/40 p-6 sm:p-8 rounded-2xl border border-border/50">
              <ul className="space-y-5 text-muted-foreground text-lg">
                <li className="flex gap-4 items-start"><FaClock className="text-primary mt-1.5 flex-shrink-0" size={18} /> <span>The Editorial Board is committed to reverting with the final decision within <strong className="text-foreground">8-10 weeks</strong> of receiving the submission.</span></li>
...
SubmissionGuidelinesJournal.displayName = 'SubmissionGuidelinesJournal'
